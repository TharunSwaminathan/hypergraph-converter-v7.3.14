...
export function buildV2VBounded(hes) {
  ...
  if (!result.ok) {
    return {
      ...
      limits: {
        maxPairs: result.budgets?.maxEstimatedPairs ?? result.budget,
        effectiveMaxRows: result.budget,
        maxRows: PROJECTION_BUDGETS.maxOutputRows,
        maxAdjacencyReferences: PROJECTION_BUDGETS.maxAdjacencyReferences,
      },
      reason: result.reason ?? `estimated ${result.estimatedPairs.toLocaleString()} candidate pairs exceeds the ${result.budget.toLocaleString()} safety limit`,
    };
  }
  return {
    ok: true,
    ...
    estimate: estimateProjectionPairCount(hes),
    limits: {
      maxPairs: PROJECTION_BUDGETS.maxEstimatedPairs,
      maxRows: PROJECTION_BUDGETS.maxOutputRows,
      maxAdjacencyReferences: PROJECTION_BUDGETS.maxAdjacencyReferences,
    },
    reason: null,
